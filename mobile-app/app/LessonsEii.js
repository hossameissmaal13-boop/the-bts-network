...
export default function LessonsEii({ navigation, route }) {
  ...
  const fetchData = async () => {
    const res1 = await getLessonsByFiliereAndAnnee("EII", "1");
    const res2 = await getLessonsByFiliereAndAnnee("EII", "2");
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.wrapper}>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <Text style={styles.pageTitle}>Lessons EII</Text>

          {renderSection(
            "Électronique et Informatique Industrielle - Première année",
            firstYear,
            "1"
          )}

          {renderSection(
            "Électronique et Informatique Industrielle - Deuxième année",
            secondYear,
            "2"
          )}
        </ScrollView>
        ...
        {/* ✅ barre de service لتحت */}
        <View style={styles.bottomBar}>
          <ServicesRow
            navigation={navigation}
            currentScreen="LessonsEii"
            student={student}
          />
        </View>
      </View>
