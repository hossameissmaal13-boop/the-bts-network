...
export default function RegisterStep1({ navigation }) {  
  ...
  const handleVerify = async () => {  
    ...
    if (typeBTS === "BTS Connecter" && !formData.codeMassar) newErrors.codeMassar = true;  
    ...
    if (typeBTS === "Libre") {  
      Alert.alert(t.success, t.noVerification);  
      setStep(2);  
    ...
    try {  
      ...
      if (res && res.success) {  
        ...
        Keyboard.dismiss();  
        Alert.alert(t.error, t.incorrectInfo);  
        setShowReport(true); // زر Report يبان  
      }  
    } catch (error) {  
      console.log("Error verifying student:", error);  
      Keyboard.dismiss();  
      Alert.alert(t.error, t.serverProblem);  
      setShowReport(true); // زر Report يبان حتى فحال كان خطأ فالخادم  
    }  
  };  
    ...
    >  
      <ScrollView contentContainerStyle={styles.container}>  
        ...
        <TextInput  
          ...
          onChangeText={(t2) => handleChange("nomFr", t2)}  
        ...
        <TextInput  
          ...
          onChangeText={(t2) => handleChange("prenomFr", t2)}  
        ...
        {/* MASSAR */}  
        {typeBTS === "BTS Connecter" && (  
          <>  
            <Text style={styles.label}>{t.massar}</Text>  
            <TextInput  
              value={formData.codeMassar}  
              onChangeText={(t2) => handleChange("codeMassar", t2)}  
        ...
        <View style={[styles.pickerWrapper, focusField === "filiere" && styles.focused]}>  
          <Picker  
            ...
            onValueChange={(t2) => handleChange("filiere", t2)}  
          >  
        ...
        <TouchableOpacity  
          ...
          onPress={() => { setShowDatePicker(true); setFocusField("date"); }}  
        >  
          <Text style={{ color: formData.dateNaissance ? "#000" : "#999" }}>{formData.dateNaissance || t.birthDatePlaceholder}</Text>  
        </TouchableOpacity>  
        {showDatePicker && (  
          <DateTimePicker  
            value={new Date()}  
            mode="date"  
            onChange={(e, d) => { setShowDatePicker(false); setFocusField(null); if (d) {  
        ...
        <View style={[styles.pickerWrapper, focusField === "year" && styles.focused]}>  
          <Picker  
            ...
            onValueChange={(t2) => handleChange("anneeScolaire", t2)}  
          >  
