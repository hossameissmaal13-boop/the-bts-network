...
export default function LessonsTc({ navigation, route }) {
  ...
  const fetchData = async () => {
    const res1 = await getLessonsByFiliereAndAnnee("TC", "1");
    const res2 = await getLessonsByFiliereAndAnnee("TC", "2");
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.wrapper}>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <Text style={styles.pageTitle}>Lessons TC</Text>

          {renderSection(
            "Technico-Commercial - Première année",
            firstYear,
            "1"
          )}

          {renderSection(
            "Technico-Commercial - Deuxième année",
            secondYear,
            "2"
          )}
        </ScrollView>
