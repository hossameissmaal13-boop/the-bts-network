...
export default function LessonsCg({ navigation, route }) {
  ...
  const fetchData = async () => {
    const res1 = await getLessonsByFiliereAndAnnee("CG", "1");
    const res2 = await getLessonsByFiliereAndAnnee("CG", "2");
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.wrapper}>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <Text style={styles.pageTitle}>Lessons CG</Text>

          {renderSection(
            "Comptabilité et Gestion - Première année",
            firstYear,
            "1"
          )}

          {renderSection(
            "Comptabilité et Gestion - Deuxième année",
            secondYear,
            "2"
          )}
        </ScrollView>
